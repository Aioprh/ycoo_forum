...
class NativeCommentList extends StatelessWidget {
  ...
  static int _extractUid(dom.Element card, dom.Element? author) {
    ...
    for (final node in nodes) {
      final raw =
          '${node.attributes['href'] ?? ''} ${node.attributes['src'] ?? ''} ${node.attributes['data-src'] ?? ''}';
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) => _CommentCard(
        ...
        onReply:
            onReply ??
            (pid, author) => _replyDialog(context, tid, fid, pid, author),
  ...
  Future<void> _openProfile(BuildContext context, _CommentFloor comment) async {
    ...
    if (uid <= 0 && comment.author.trim().isNotEmpty) {
      ...
      uid =
          await CommentProfileResolver.instance.resolveUid(comment.author) ?? 0;
    ...
    if (uid <= 0) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('未找到该用户资料，请稍后重试')));
      return;
    }
  ...
  Future<void> _replyDialog(
    ...
    if (tid <= 0 || fid <= 0 || pid <= 0) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('未取得楼层信息，请刷新帖子后重试')));
      return;
    }
    await AuthService.instance.init();
    if (!AuthService.instance.isLoggedIn) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('请先登录后回复')));
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: s.shadow.withValues(alpha: .035),
            blurRadius: 8,
          ...
          Row(
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                  comment.floor.isEmpty ? '${index + 2}楼' : comment.floor,
                  style: TextStyle(
                    fontSize: 10,
          ...
          if (comment.pid > 0)
            Align(
              ...
              child: TextButton.icon(
                onPressed: () => onReply(comment.pid, comment.author),
...
class _HtmlNodes extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: element.nodes.map((n) => _node(context, n)).toList(),
  );
  ...
  Widget _node(BuildContext context, dom.Node node) {
    ...
    if (tag == 'blockquote')
      return Container(
        ...
        decoration: BoxDecoration(
          color: s.primaryContainer.withValues(alpha: .42),
          borderRadius: BorderRadius.circular(10),
          border: Border(left: BorderSide(color: s.primary, width: 3)),
        ),
        child: _HtmlNodes(element: node),
      );
    if (tag == 'pre' || tag == 'code')
      return Container(
        width: double.infinity,
