...
class _MemberFeaturePageState extends State<MemberFeaturePage> {
  ...
  Widget build(BuildContext context) {
    if (widget.type == MemberFeatureType.notices)
      return const NativeNoticeHubPage();
    return Scaffold(
      ...
      body: FutureBuilder<Object>(
        ...
        builder: (context, snapshot) {
          ...
          if (value is List<NativeMessage>)
            return _MessageList(items: value, onCompose: _openSendPm);
...
class _ThreadList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty)
      return const _EmptyState(title: '暂无帖子', subtitle: '这里还没有相关主题');
      ...
      itemBuilder: (context, i) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Text(
              item.boardName.isEmpty ? '主题 #${item.tid}' : item.boardName,
            ),
...
class _MessageList extends StatelessWidget {
  ...
  String _sender(NativeMessage item) {
    if (item.sender.trim().isNotEmpty && item.sender != '站内私信')
      return item.sender.trim();
    final title = item.title.trim();
    final m = RegExp(r'^(?:来自|发自|消息来自)\s*[:：]?\s*(.+)$').firstMatch(title);
    return m?.group(1)?.trim().isNotEmpty == true
        ? m!.group(1)!.trim()
        : (title.isEmpty ? '站内私信' : title);
  }
            ...
            children: [
              Row(
                children: [
                  CircleAvatar(
                    radius: 23,
                    child: Text(_sender(item).characters.first),
                  ),
              ...
              Container(
                ...
                child: Text(item.subtitle, style: const TextStyle(height: 1.5)),
              ),
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty) {
      return Column(
        children: [
          const Expanded(
            child: _EmptyState(title: '暂无私信', subtitle: '和其他用户的站内私信会显示在这里'),
          ),
          Padding(
                ...
                const Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 3),
                      Text('点击卡片查看消息内容', style: TextStyle(fontSize: 11)),
                    ],
                  ),
                ),
                IconButton.filled(
        ...
        return Card(
          ...
          child: ListTile(
            leading: CircleAvatar(
              ...
              child: Text(sender.characters.first),
            ),
            title: Text(
              sender,
              maxLines: 1,
...
class _FriendList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty)
      return const _EmptyState(title: '暂无好友或关注', subtitle: '你的好友、关注与粉丝会显示在这里');
    return ListView.separated(
      padding: const EdgeInsets.all(14),
      itemCount: items.length,
      separatorBuilder: (_, _) => const SizedBox(height: 8),
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('余额', style: TextStyle(fontSize: 13)),
