...
class _CreateThreadPageState extends State<CreateThreadPage> {
    ...
    try {
      ...
      for (var i = 0; i < picked.files.length; i++) {
        ...
        if (file.size > AttachmentUploadService.maxBytes) {
          if (mounted) setState(() => _error = '${file.name} 超过 ${(AttachmentUploadService.maxBytes / 1024 / 1024).toStringAsFixed(0)} MB，已跳过');
          continue;
        }
  ...
  Widget _attachmentCard(BuildContext context) {
    ...
    return Container(decoration: BoxDecoration(color: scheme.surface, borderRadius: BorderRadius.circular(22), border: Border.all(color: scheme.outlineVariant.withOpacity(.65))), padding: const EdgeInsets.fromLTRB(16, 14, 16, 12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text('点按缩略图插入正文 · 长按拖动可调整顺序 · 单个文件最大 ${(AttachmentUploadService.maxBytes / 1024 / 1024).toStringAsFixed(0)} MB', style: const TextStyle(fontSize: 12)),
