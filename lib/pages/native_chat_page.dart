...
class _NativeChatPageState extends State<NativeChatPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final seen = <String>{};
      final nodes = doc.querySelectorAll('dl[id^="pmlist_"]');
      ...
      for (final node in nodes) {
        ...
        final key = id == null
            ? '${message.uid}|${message.sender}|${message.body}|${message.time}'
            : 'id:$id';
      ...
      if (nodes.isEmpty) {
        for (final node in doc.querySelectorAll(
          'li.comiis_pmitem,li.pm_list,li.pml,.comiis_pm_list > li,.pmlist > li',
        )) {
          final message = _parseMessage(node);
          if (message == null) continue;
          final key = '${message.uid}|${message.sender}|${message.body}|${message.time}';
          if (seen.add(key)) result.add(message);
        }
      }
      ...
      if (mounted) {
        setState(() {
          _error = _clean(e.toString().replaceFirst('Exception: ', ''));
          _loading = false;
        });
      }
    }
  ...
  _ChatMessage? _parseMessage(dynamic node) {
    final content = node.querySelector('dd.ptm') ??
        node.querySelector(
          '.pm_message,.pml_body,.pm_body,.comiis_pmtext,.comiis_pm_content,.pmtext',
        );
    if (content == null) return null;

    final author = node.querySelector(
      'a[href*="mod=space&uid="],a[href*="mod=space%26uid="],a[href*="uid="]',
    );
    ...
    final timeNode = node.querySelector(
      '.xg1,.xg2,time,[class*="time"],[class*="date"]',
    );
    ...
    final finalSender = sender.isNotEmpty
        ? sender
        : (uid == AuthService.instance.uid ? currentName : _clean(widget.username));

    return _ChatMessage(
      uid: uid,
      sender: finalSender.isEmpty ? _clean(widget.username) : finalSender,
      body: body,
      time: time,
    );
  }
  ...
  static int _extractUid(dynamic node) {
    final links = node.querySelectorAll(
      'a[href*="mod=space&uid="],a[href*="mod=space%26uid="],a[href*="?uid="],a[href*="&uid="],a[href*="uid="]',
    );
  ...
  static String _removeFirst(String source, String value) {
    final target = value.trim();
    if (target.isEmpty) return source;
    final index = source.indexOf(target);
    if (index < 0) return source;
    return '${source.substring(0, index)}${source.substring(index + target.length)}'.trim();
  }
  ...
  Future<void> _send() async {
    ...
    final error = await MemberServiceV2.instance.sendMessage(
      to: widget.username,
      message: text,
    );
    ...
    if (error != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(forumText(error))),
      );
      return;
    }
  ...
  bool _isMine(_ChatMessage message) {
    ...
    if (currentUid != null && currentUid > 0 && message.uid > 0) {
      return currentUid == message.uid;
    }
    final currentName = _clean(AuthService.instance.username ?? '');
  ...
  Widget _avatar(int uid) {
    ...
    return CircleAvatar(
      backgroundImage: NetworkImage(
        '${SiteConfig.base}uc_server/avatar.php?uid=$uid&size=small',
      ),
    );
  }
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return Padding(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      if (!mine) ...[
                                        _avatar(message.uid > 0 ? message.uid : widget.uid),
                                      ...
                                      Flexible(
                                        child: Container(
                                          ...
                                          decoration: BoxDecoration(
                                            color: mine ? scheme.primaryContainer : Colors.white,
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              Text(message.body, style: const TextStyle(fontSize: 15, height: 1.45)),
