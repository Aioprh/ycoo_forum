...
class _DetailPageState extends State<DetailPage> {
  ...
  Future<void> _login() async {
    ...
    if (ok == true && mounted) {
      await AuthService.instance.init(); await AuthService.instance.checkLoggedIn();
      if (!mounted) return; setState(() => _loggedIn = AuthService.instance.isLoggedIn); await _fetch();
    }
  }
  ...
  Future<void> _reward(ThreadDetail d) async {
    ...
    final amount = await showDialog<int>(context: context, builder: (c) => AlertDialog(
      title: const Text('打赏作者'),
      content: TextField(controller: input, autofocus: true, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: '打赏数量', suffixText: '积分', border: OutlineInputBorder(borderRadius: BorderRadius.circular(14)))),
      actions: [TextButton(onPressed: () => Navigator.pop(c), child: const Text('取消')), FilledButton(onPressed: () { final v = int.tryParse(input.text.trim()) ?? 0; if (v > 0) Navigator.pop(c, v); }, child: const Text('确认打赏'))],
    ));
    input.dispose(); if (amount == null || !mounted) return;
    ...
    if (!mounted) return; setState(() => _rewarding = false); _snack(result ?? '打赏成功');
  }
  ...
  Future<void> _purchase() async {
    ...
    final ok = await showDialog<bool>(context: context, builder: (c) => AlertDialog(
      title: const Text('购买主题'), content: Text('确定购买这个付费主题吗？\n\n价格：$price'),
      actions: [TextButton(onPressed: () => Navigator.pop(c, false), child: const Text('取消')), FilledButton(onPressed: () => Navigator.pop(c, true), child: const Text('确定购买'))],
    )) ?? false;
    if (!ok || !mounted) return; setState(() => _buying = true);
    final result = await ApiService.instance.purchaseThread(d.tid);
    if (!mounted) return; setState(() => _buying = false); _snack(result.message); if (result.success) await _fetch();
  }

  @override
  Widget build(BuildContext context) {
    final colors = Theme.of(context).colorScheme;
    return Scaffold(backgroundColor: colors.surfaceContainerLowest,
      appBar: AppBar(titleSpacing: 8, leading: IconButton(onPressed: () => Navigator.maybePop(context), icon: const Icon(Icons.arrow_back_rounded)),
        ...
        actions: [IconButton(onPressed: _fetch, icon: const Icon(Icons.refresh_rounded))], backgroundColor: colors.surfaceContainerLowest, surfaceTintColor: Colors.transparent, scrolledUnderElevation: 0),
      body: _buildBody(context), bottomNavigationBar: _composer(context));
  }
  ...
  Widget _errorView(BuildContext context) {
    final c = Theme.of(context).colorScheme;
    return Center(child: Padding(padding: const EdgeInsets.all(32), child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.cloud_off_rounded, size: 48, color: c.onSurfaceVariant), const SizedBox(height: 14), const Text('详情加载失败', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16)), const SizedBox(height: 7), Text('网络或服务器暂时不可用', style: TextStyle(color: c.onSurfaceVariant)), const SizedBox(height: 16), FilledButton.icon(onPressed: _fetch, icon: const Icon(Icons.refresh_rounded), label: const Text('重新加载'))])));
  }

  Widget _hero(BuildContext context, ThreadDetail d) {
    final c = Theme.of(context).colorScheme;
    return Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.fromLTRB(18, 18, 18, 16), decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [c.primaryContainer.withValues(alpha: .72), c.surface]), borderRadius: BorderRadius.circular(26), border: Border.all(color: c.outlineVariant.withValues(alpha: .45))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      if (d.boardName.isNotEmpty) GestureDetector(onTap: d.fid == 0 ? null : () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => BoardThreadListPage(filter: d.boardName, fid: d.fid))), child: _tag(context, Icons.forum_rounded, d.boardName)),
      ...
      Row(children: [CircleAvatar(radius: 22, backgroundColor: c.primaryContainer, backgroundImage: d.avatar.isNotEmpty ? NetworkImage(d.avatar) : null, child: d.avatar.isNotEmpty ? null : Icon(Icons.person_rounded, color: c.primary)), const SizedBox(width: 11), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Flexible(child: Text(d.author, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w800))), if (d.level.isNotEmpty) ...[const SizedBox(width: 7), _chip(d.level, c.secondaryContainer, c.onSecondaryContainer)]]), if (d.time.isNotEmpty) ...[const SizedBox(height: 3), Text(d.time, style: TextStyle(fontSize: 11.5, color: c.onSurfaceVariant))]]))]),
  ...
  Widget _tag(BuildContext context, IconData icon, String text) { final c = Theme.of(context).colorScheme; return Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: c.surface.withValues(alpha: .75), borderRadius: BorderRadius.circular(12)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 15, color: c.primary), const SizedBox(width: 6), Text(text, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: c.primary)), const SizedBox(width: 3), Icon(Icons.chevron_right_rounded, size: 15, color: c.primary)])); }
  ...
  Widget _metaChip(BuildContext context, IconData icon, String text) { final c = Theme.of(context).colorScheme; return Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 6), decoration: BoxDecoration(color: c.surface.withValues(alpha: .72), borderRadius: BorderRadius.circular(10), border: Border.all(color: c.outlineVariant.withValues(alpha: .35))), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 14, color: c.onSurfaceVariant), const SizedBox(width: 5), Text(text, style: TextStyle(fontSize: 11, color: c.onSurfaceVariant, fontWeight: FontWeight.w600))])); }

  Widget _quickActions(BuildContext context, ThreadDetail d) {
    final c = Theme.of(context).colorScheme;
    Widget item(IconData icon, String label, {bool active = false, int? count, required VoidCallback onTap}) => Expanded(child: InkWell(borderRadius: BorderRadius.circular(14), onTap: onTap, child: Padding(padding: const EdgeInsets.symmetric(vertical: 10), child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 20, color: active ? c.primary : c.onSurfaceVariant), const SizedBox(height: 4), Text(count != null && count > 0 ? '$label · $count' : label, style: TextStyle(fontSize: 11.5, fontWeight: active ? FontWeight.w700 : FontWeight.w600, color: active ? c.primary : c.onSurfaceVariant))]))));
    return Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 3), decoration: BoxDecoration(color: c.surface, borderRadius: BorderRadius.circular(18), border: Border.all(color: c.outlineVariant.withValues(alpha: .45))), child: Row(children: [item(_liked ? Icons.favorite_rounded : Icons.favorite_border_rounded, _liked ? '已点赞' : '点赞', active: _liked, count: _likeCount, onTap: () => _like(d)), item(_favorited ? Icons.star_rounded : Icons.star_border_rounded, _favorited ? '已收藏' : '收藏', active: _favorited, onTap: () => _favorite(d)), item(_rewarding ? Icons.hourglass_top_rounded : Icons.card_giftcard_outlined, '打赏', active: _rewarding, onTap: () => _reward(d)), item(Icons.chat_bubble_outline_rounded, '回复', onTap: () => _replyFocus.requestFocus())]));
  }
  ...
  Widget _commentsSection(BuildContext context, ThreadDetail d) { final c = Theme.of(context).colorScheme; return Container(margin: const EdgeInsets.only(top: 2, bottom: 10), decoration: BoxDecoration(color: c.surface, borderRadius: BorderRadius.circular(24), border: Border.all(color: c.outlineVariant.withValues(alpha: .45))), child: Column(children: [InkWell(onTap: () => setState(() => _commentsExpanded = !_commentsExpanded), child: Padding(padding: const EdgeInsets.fromLTRB(16, 15, 12, 15), child: Row(children: [const Expanded(child: Text('评论 / 回复', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800))), Icon(_commentsExpanded ? Icons.keyboard_arrow_up_rounded : Icons.keyboard_arrow_down_rounded)]))), if (_commentsExpanded) ...[Divider(height: 1, color: c.outlineVariant.withValues(alpha: .35)), NativeCommentList(html: d.commentsHtml)]])); }
  Widget _empty(BuildContext context, String text) { final c = Theme.of(context).colorScheme; return Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.symmetric(vertical: 44), decoration: BoxDecoration(color: c.surface, borderRadius: BorderRadius.circular(22), border: Border.all(color: c.outlineVariant.withValues(alpha: .4))), child: Column(children: [Icon(Icons.article_outlined, size: 34, color: c.onSurfaceVariant), const SizedBox(height: 9), Text(text, style: TextStyle(color: c.onSurfaceVariant))])); }

  Widget _composer(BuildContext context) {
    final c = Theme.of(context).colorScheme, d = _detail;
    if (d == null || d.isPaid) return const SizedBox.shrink();
    return SafeArea(top: false, child: Container(padding: const EdgeInsets.fromLTRB(12, 8, 12, 8), decoration: BoxDecoration(color: c.surface, border: Border(top: BorderSide(color: c.outlineVariant.withValues(alpha: .45))), boxShadow: [BoxShadow(color: c.shadow.withValues(alpha: .045), blurRadius: 16, offset: const Offset(0, -4))]), child: !_loggedIn ? OutlinedButton.icon(onPressed: _login, icon: const Icon(Icons.login_rounded, size: 18), label: const Text('登录后参与讨论'), style: OutlinedButton.styleFrom(minimumSize: const Size.fromHeight(46), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)))) : Row(crossAxisAlignment: CrossAxisAlignment.end, children: [Expanded(child: TextField(controller: _replyCtrl, focusNode: _replyFocus, minLines: 1, maxLines: 4, textInputAction: TextInputAction.send, onSubmitted: (_) => _reply(), decoration: InputDecoration(hintText: '友善地说点什么…', isDense: true, filled: true, fillColor: c.surfaceContainerHighest.withValues(alpha: .72), border: OutlineInputBorder(borderRadius: BorderRadius.circular(18), borderSide: BorderSide.none), contentPadding: const EdgeInsets.symmetric(horizontal: 15, vertical: 11)))), const SizedBox(width: 7), SizedBox(width: 46, height: 46, child: FilledButton(onPressed: _sending ? null : _reply, style: FilledButton.styleFrom(padding: EdgeInsets.zero, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))), child: _sending ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.arrow_upward_rounded)))]]));
  }
}
