...
<base href="https://www.ycoo.net/"><style>
*{box-sizing:border-box}html,body{margin:0;padding:0;background:transparent;color:#20232b;overflow-wrap:anywhere;-webkit-font-smoothing:antialiased}
p{margin:0 0 13px}h1,h2,h3,h4{line-height:1.4;margin:20px 0 10px;color:#12151c}a{color:#4d63d8;text-decoration:none}strong,b{color:#12151c}
ul,ol{padding-left:22px}li{margin:5px 0}img{display:block!important;width:auto!important;max-width:100%!important;height:auto!important;max-height:72vh!important;object-fit:contain!important;border-radius:14px;margin:14px auto!important}video,iframe{max-width:100%!important;border-radius:14px}table{width:100%!important;max-width:100%!important;border-collapse:collapse;margin:12px 0}td,th{padding:8px;overflow-wrap:anywhere;border:1px solid #e1e4ea}pre,code{white-space:pre-wrap;word-break:break-word}code{background:#f0f2f6;padding:2px 6px;border-radius:6px}pre{background:#f3f4f7;padding:14px;border-radius:12px;overflow:hidden}blockquote{margin:14px 0;padding:10px 14px;border-left:3px solid #7184e8;background:#f4f6ff;color:#687080;border-radius:0 12px 12px 0}.attachment-link{display:flex!important;align-items:center;gap:10px;padding:13px 14px;margin:10px 0;background:#f4f6ff;border:1px solid #dce2ff;border-radius:12px;color:#4058d6!important;font-weight:650}
  ...
  Future<void> _reward(ThreadDetail d) async {
    ...
    final amount = await showDialog<int>(context: context, builder: (c) => AlertDialog(
      ...
      content: TextField(controller: controller, autofocus: true, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: '打赏数量', suffixText: '积分', border: OutlineInputBorder(borderRadius: BorderRadius.circular(14)))),
      actions: [TextButton(onPressed: () => Navigator.pop(c), child: const Text('取消')), FilledButton(onPressed: () { final value = int.tryParse(controller.text.trim()) ?? 0; if (value > 0) Navigator.pop(c, value); }, child: const Text('确认打赏'))],
    ));
    controller.dispose();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        titleSpacing: 8,
        leading: IconButton(tooltip: '返回', onPressed: () => Navigator.maybePop(context), icon: const Icon(Icons.arrow_back_rounded)),
        ...
        actions: [IconButton(tooltip: '刷新', onPressed: _fetch, icon: const Icon(Icons.refresh_rounded)), const SizedBox(width: 4)],
        backgroundColor: s.surfaceContainerLowest,
  ...
  Widget _errorView(BuildContext context) => Center(child: Padding(padding: const EdgeInsets.all(32), child: Column(mainAxisSize: MainAxisSize.min, children: [
    Icon(Icons.cloud_off_rounded, size: 48, color: Theme.of(context).colorScheme.onSurfaceVariant),
    const SizedBox(height: 14), const Text('详情加载失败', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16)), const SizedBox(height: 7),
    Text('网络或服务器暂时不可用', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)), const SizedBox(height: 16),
  ...
  Widget _hero(BuildContext context, ThreadDetail d) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [s.primaryContainer.withValues(alpha: .72), s.surface]),
        borderRadius: BorderRadius.circular(26),
        border: Border.all(color: s.outlineVariant.withValues(alpha: .45)),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        if (d.boardName.isNotEmpty) GestureDetector(
          ...
          child: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: s.surface.withValues(alpha: .75), borderRadius: BorderRadius.circular(12)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.forum_rounded, size: 15, color: s.primary), const SizedBox(width: 6), Text(d.boardName, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: s.primary)), const SizedBox(width: 3), Icon(Icons.chevron_right_rounded, size: 15, color: s.primary)])),
        ),
        const SizedBox(height: 14),
        Text(d.title, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontSize: 23, height: 1.28, fontWeight: FontWeight.w850)),
        const SizedBox(height: 16),
        Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
          CircleAvatar(radius: 22, backgroundColor: s.primaryContainer, backgroundImage: d.avatar.isNotEmpty ? NetworkImage(d.avatar) : null, child: d.avatar.isNotEmpty ? null : Icon(Icons.person_rounded, color: s.primary)),
          const SizedBox(width: 11),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [Flexible(child: Text(d.author, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w800))), if (d.level.isNotEmpty) ...[const SizedBox(width: 7), _chip(context, d.level, s.secondaryContainer, s.onSecondaryContainer)] ]),
        ...
        Wrap(spacing: 7, runSpacing: 7, children: [
          _metaChip(context, Icons.visibility_outlined, '主题 ${d.tid}'),
  ...
  Widget _chip(BuildContext context, String text, Color bg, Color fg) => Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2), decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(8)), child: Text(text, style: TextStyle(fontSize: 10, color: fg, fontWeight: FontWeight.w600)));
  ...
  Widget _sectionHeader(BuildContext context, String title, String subtitle, IconData icon) {
    final s = Theme.of(context).colorScheme;
    return Padding(padding: const EdgeInsets.fromLTRB(4, 7, 4, 10), child: Row(children: [Container(width: 38, height: 38, decoration: BoxDecoration(color: s.primaryContainer, borderRadius: BorderRadius.circular(12)), child: Icon(icon, size: 20, color: s.primary)), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w850)), const SizedBox(height: 2), Text(subtitle, style: TextStyle(fontSize: 11.5, color: s.onSurfaceVariant))]))]));
  }
  ...
  Widget _paidNotice(BuildContext context, ThreadDetail d) {
    final s = Theme.of(context).colorScheme;
    return Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(26), decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [s.primaryContainer, s.surface]), borderRadius: BorderRadius.circular(24), border: Border.all(color: s.outlineVariant.withValues(alpha: .45))), child: Column(children: [
      Container(width: 68, height: 68, decoration: BoxDecoration(color: s.primary.withValues(alpha: .12), shape: BoxShape.circle), child: Icon(Icons.lock_rounded, size: 30, color: s.primary)), const SizedBox(height: 14),
      const Text('这是一个付费主题', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w850)), const SizedBox(height: 6), Text(d.price == null ? '购买后即可查看完整内容' : '支付 ${d.price} ${d.currency} 后查看完整内容', textAlign: TextAlign.center, style: TextStyle(color: s.onSurfaceVariant)), const SizedBox(height: 18),
  ...
  Widget _commentsSection(BuildContext context, ThreadDetail d) {
    final s = Theme.of(context).colorScheme;
    return Container(margin: const EdgeInsets.only(top: 2, bottom: 10), decoration: BoxDecoration(color: s.surface, borderRadius: BorderRadius.circular(24), border: Border.all(color: s.outlineVariant.withValues(alpha: .45))), child: Column(children: [
      InkWell(borderRadius: const BorderRadius.vertical(top: Radius.circular(24)), onTap: () => setState(() => _commentsExpanded = !_commentsExpanded), child: Padding(padding: const EdgeInsets.fromLTRB(16, 15, 12, 15), child: Row(children: [
        Container(width: 4, height: 30, decoration: BoxDecoration(color: s.secondary, borderRadius: BorderRadius.circular(4))), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('评论 / 回复', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w850)), const SizedBox(height: 2), Text('原生楼层 · 按楼层浏览', style: TextStyle(fontSize: 11.5, color: s.onSurfaceVariant))])), Container(width: 34, height: 34, decoration: BoxDecoration(color: s.surfaceContainerHighest, shape: BoxShape.circle), child: Icon(_commentsExpanded ? Icons.keyboard_arrow_up_rounded : Icons.keyboard_arrow_down_rounded, size: 21)),
      ])),
      if (_commentsExpanded) ...[Divider(height: 1, color: s.outlineVariant.withValues(alpha: .35)), NativeCommentList(html: d.commentsHtml)],
    ]));
  }
  ...
  Widget _composer(BuildContext context) {
    ...
    return SafeArea(top: false, child: Container(padding: const EdgeInsets.fromLTRB(12, 8, 12, 8), decoration: BoxDecoration(color: s.surface, border: Border(top: BorderSide(color: s.outlineVariant.withValues(alpha: .45))), boxShadow: [BoxShadow(color: s.shadow.withValues(alpha: .045), blurRadius: 16, offset: const Offset(0, -4))]), child: !_loggedIn
