...
class NetClient {
  ...
  static String? _declaredCharset(List<int> bytes) {
    ...
    return RegExp(
      ...
      caseSensitive: false,
    ).firstMatch(ascii)?.group(1)?.trim().toLowerCase();
  }
  ...
  static String? extractFormHash(String html) {
    ...
    // Raw triple-quoted string is intentional: both single and double quotes
    // may occur inside the HTML attribute without terminating the Dart string.
    for (final match in RegExp(
      r'''<[^>]*\bdata-formhash\s*=\s*["']([^"']+)["'][^>]*>''',
      caseSensitive: false,
    ).allMatches(html)) {
  ...
  static bool _validFormHash(String? value) =>
      value != null && RegExp(r'^[A-Za-z0-9_-]{6,128}$').hasMatch(value);
  ...
  static http.BaseRequest? _cloneWithToken(
    http.BaseRequest request,
    String token,
  ) {
    ...
    if (!contentType
        .toLowerCase()
        .contains('application/x-www-form-urlencoded')) return null;
  ...
  static Future<http.StreamedResponse> _sendWithRefresh(
    ...
    if (request.url.queryParameters.containsKey('formhash') ||
        request.url.queryParameters.containsKey('hash')) {
      final replacement =
          http.Request(request.method, _replaceQueryToken(request.url, token));
